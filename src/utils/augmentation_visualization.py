...
def generate_sdf_from_voxel(mesh, voxel, resolution):
    ...
    gt_sdf_voxel = gt_sdf_voxel.reshape(
...
def voxel_slice_plot(gt_sdf_voxel, number_of_slices, resolution, title):
    ...
    for dim in range(3):
        for i in range(number_of_slices):
            ...
            ax.set_axis_off()
            plot = ax.imshow(
                gt_sdf_voxel_slice, origin="lower", cmap="seismic", interpolation="none"
            )
...
def voxel_plot(
    ...
    for i in range(0, number_of_slices, 1):
        ...
        if number_of_slices + center < gt_sdf_voxel.shape[0]:
            ...
            )  # this is default; -value_limit, value_limit, for the transformer, we set it to -2, 2
            fig.colorbar(plot, ax=ax)
            fig.tight_layout()
            canvas.draw()
            image = np.array(canvas.renderer.buffer_rgba())
...
def voxel_plot_valueset(
    gt_sdf_voxel: np.array, number_of_slices: int, resolution: int, title: str
):
    min_sdf = np.min(gt_sdf_voxel)
    max_sdf = np.max(gt_sdf_voxel)
    # print("\n min_sdf: ", min_sdf, " , max_sdf: ", max_sdf)
