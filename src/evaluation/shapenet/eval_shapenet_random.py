...
class EVALShapenet:
    def __init__(
        ...
        if self.pre_trained:
            self.pre_trained_transformer = rtfc.TransformerSDFtoSDFShapenetNormalizedNoEmptyMaskingCustom.load_from_checkpoint(
    ...
    def assign_constant_learnable_tensors(
        self, non_optimized_latent_codes: torch.Tensor, bool_dict: dict
    ) -> torch.Tensor:
        masked_non_optimized_latent_codes = non_optimized_latent_codes.clone()

        mask_all_bool = bool_dict["mask_all_bool"]

        masked_non_optimized_latent_codes[mask_all_bool] = (
            self.frozen_constant_learnable_mask_tensors
        )
    ...
    def generate_all_masking(
        self, sub_voxels: torch.Tensor, object_indices: torch.Tensor, mesh_file_names
    ):
        ...
        if not torch.all(torch.any(non_empty_sub_voxels_bool, dim=1)):
            pair = {
                ...
                "mesh_file_name": mesh_file_names,
            }
            print("\nall voxels are empty", pair)
            breakpoint()

        mask_all_bool, num_mask_all = gr_mask.generate_random_mask_for_all(
            self.number_of_sub_voxels, batch_size, masking_ratio=self.masking_ratio
        )
        # just for return
    ...
    def fwd(self, batch: list) -> Tuple:
        ...
        mask_all_bool, masked_bool, non_masked_bool = self.generate_all_masking(
            sub_voxels, object_indices, mesh_file_names
        )
        # FORWARD CAlL-----------------------------------------------------------------------------------------------------------------------------
    ...
    def evaluate_val(self):
        ...
        for i in tqdm(range(min_range, max_range, 1), desc="Val Samples"):
            ...
            mesh_info, metric_info = calculate_metric_scale_for_mesh_file_name(
                mesh_file_names, self.bbx_list
            )
