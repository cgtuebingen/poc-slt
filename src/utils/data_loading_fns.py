...
def fetch_latent_codes_from_32cubelmdb(
    dict_for_latent_codes: dict,
    lmdb_optimized_latent_codes_obj,
    number_of_sub_voxels: int,
    latent_dim: int,
    val_batch_size: int,
) -> torch.Tensor:
    optimized_latent_codes = torch.zeros(
        [val_batch_size, number_of_sub_voxels, latent_dim, 2, 2, 2],
        dtype=torch.float32,
        device="cuda",
    )
    ...
    for b in range(val_batch_size):
        ...
        key, object_index_lmdb, mesh_file_name_lmdb, optimized_latent_code = (
            lmdb_optimized_latent_codes_obj[object_index]
        )
        optimized_latent_code = optimized_latent_code.to(device="cuda")
        assert (
            object_index == object_index_lmdb
        )  # since we chose a few selected meshes then they are not going to be the same
