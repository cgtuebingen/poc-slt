import torch
def normalize_voxels_to_their_distance_with_batch(voxels: torch.Tensor, distances_per_voxel: torch.Tensor, batch_size: int, target_resolution: int) -> torch.Tensor:
    assert (voxels.shape) == (batch_size, target_resolution, target_resolution, target_resolution)
    assert (distances_per_voxel.shape) == (batch_size, 1)
    normalized_voxels = torch.zeros(voxels.shape, dtype=torch.float32, device=torch.device('cuda'))
...
def normalize_voxels_to_their_distance(voxel: torch.Tensor, distances_per_voxel: torch.Tensor, target_resolution: int) -> torch.Tensor:
    ...
    if (torch.min(normalized_voxel) < -torch.sqrt(torch.as_tensor(3)) or torch.max(normalized_voxel) > torch.sqrt(torch.as_tensor(3))):
        raise "normalization is wrong"
